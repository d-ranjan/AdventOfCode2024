...
constexpr std::array operaters {add, multiply, concat};

[[nodiscard]] constexpr static auto concat_val(const size_t first, const size_t second) -> size_t
{
    return std::stoll(fmt::format("{}{}", first, second));
}
                                                     ...
                                                     const size_t               result) -> size_t
{
    switch (values.size()) {
        case 2: {
            if (values.front() * values.back() == result || values.front() + values.back() == result
                || concat_val(values.front(), values.back()) == result) {
                return result;
            }
            break;
        }
        case 3: {
            for (auto&& operations_tie : std::views::cartesian_product(operaters, operaters)) {
                ...
                for (size_t i = 1; i < values.size(); ++i) {
                    if (operations.at(i - 1) == add) {
                        res += values.at(i);
                    } else if (operations.at(i - 1) == multiply) {
                        res *= values.at(i);
                    } else {
                        res = concat_val(res, values.at(i));
                    }
                }
                if (res == result) {
                    return result;

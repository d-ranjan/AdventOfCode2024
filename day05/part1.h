...
[[nodiscard]] constexpr static auto get_result(const std::string_view document) -> int
{
    ...
    return std::ranges::fold_left(document | std::views::split('\n'), 0,
        ...
        if (std::ranges::contains(line, '|')) {
            ...
            // fmt::println("{}", page_ordering_rules);
        } else if (std::ranges::contains(line, ',')) {
            const auto update_page_numbers
            ...
            const auto is_sorted
                = std::ranges::is_sorted(update_page_numbers, [&](auto&& left, auto&& right) {
                return std::ranges::none_of(page_ordering_rules, [&](auto&& page_order){
            ...
            // fmt::println("{} => sorted: {}", update_page_numbers, is_sorted);
        }
        return sum;
    });
}
